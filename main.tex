...
We show that our approach can generate designs that are exponentially large in the size of their specifications.  In addition to that significant design leverage, the resulting finite-state machines enjoy all of the closure properties and proof opportunities for regular languages.
\end{abstract}
...
We consider a basic two-player (Figure~\ref{fig:nimPlayerFSM}) game using a single, subtractive heap of 5 tokens (Figure~\ref{fig:nimHeapFSM}\pnote{make [0] accepting}).
...
We prefer to process the inputs as (A,2), (B,1), and (A,2), so that the player making the move and the number of subtracted tokens are processed in lock step.  In this way, any actions associated with each move will be properly taken upon that move.  Similarly, if the game had 3 heaps in play instead of a single heap, we want all heaps to be modified by each turn.  Concatenation focus on one heap until it is exhausted before moving to the next heap.  \pnote{focuses or focus?}  Finally, consider that the first machine accepts~ABAB which would be inappropriate if the second machine accepts~212, since only~3 moves are made.  The lock-step execution we require is over inputs for each machine that are the same length.
...
\rnote{I am not sure this section is necessary.  We can point to github for the code.}
...
% Features &States &Tokens &Transitions \\\midrule
% H=21,P=1,A=[1],M &24 &3 &72 \\
% H=21,P=2,A=[1],M &24 &4 &96 \\
% H=21,P=4,A=[1],M &24 &6 &144 \\
% H=21,P=1,A=[1,2],M &24 &4 &96 \\
% H=21,P=2,A=[1,2],M &45 &6 &270 \\
% H=21,P=4,A=[1,2],M &81 &10 &810 \\
% H=21,P=1,A=[1,2,3],M &24 &5 &120 \\
% H=21,P=2,A=[1,2,3],M &45 &8 &360 \\
% H=21,P=4,A=[1,2,3],M &83 &14 &1162 \\
% \bottomrule
% \end{tabular}
% \caption{Number of generated states, tokens, and transitions depending on selected features for 21. The features are as follows: H = Add Heap, P = Players, A = legal quantities of tokens to add for 21, and M = Mis\`{e}re play. }\label{tab:21Nim }
% \end{table}
...
% H=3,T &1 &1 &1 \\
% H=3,P=1,T &5 &5 &25 \\
% H=[3,4],P=1,T &21 &9 &189 \\
% H=[3,4,5],P=1,T &121 &14 &1694 \\
% H=3,P=2,T &7 &7 &49 \\
% H=[3,4],P=2,T &37 &16 &592 \\
% H=[3,4,5],P=2,T &233 &26 &6058 \\
% H=3,P=4,T &8 &8 &64 \\
% H=[3,4],P=4,T &56 &30 &1680 \\
% H=[3,4,5],P=4,T &415 &50 &20750 \\
% H=3,P=1,T,M &6 &5 &30 \\
% H=3,P=2,T,M &9 &7 &63 \\
% H=3,P=4,T,M &11 &8 &88 \\
% H=[3,4],P=1,T,M &22 &9 &198 \\
% H=[3,4],P=2,T,M &39 &16 &624 \\
% H=[3,4],P=4,T,M &60 &30 &1800 \\
% H=[3,4,5],P=1,T,M &112 &14 &1708 \\
% H=[3,4,5],P=2,T,M &235 &26 &6110 \\
% H=[3,4,5],P=4,T,M &419 &50 &20950 \\
% \bottomrule
% \end{tabular}
% \caption{Number of generated states, tokens, and transitions depending on selected features for traditional Nim. The features are as follows: H = Add Heap, P = Players, T = traditional Nim, and M = Mis\`{e}re play. }\label{tab:traditionalNim }
% \end{table}


\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/nimexample/heapFSM.pdf}
