...
The Subtraction game is a single heap variation of Nim, usually played by a small group of players. In our analysis, we fix the starting number of sticks to 21 each game. Each turn, the current player removes between one and a set number of sticks from the heap, with the game ending when the final stick is removed. 
...
\bottomrule
\end{tabular}
\caption{Number of generated states and transitions for the Subtraction game. Every game is played with a single starting heap of 21 sticks. \pnote{Re-iterate the starting quantity of sticks here? or just leave that in the above paragraph?}}\label{tab:subtractionNim}
\end{figure}
