...
\pnote{Should these be done as ordered pairs of states, like in a ''cross product" -- e.g., $(q_A, q_B)$? Or should these be done in the way Ron's done on the board, such as $q_Aq_B$?}
...
\STATE{$L \gets \{q_C\}$} \hfill\COMMENT{States to be processed.}
\STATE{$P \gets \emptyset$} \hfill\COMMENT{Already-processed states.}
\WHILE{$L \neq \emptyset$}
    \STATE{$s = (s_A, s_B) \gets \mathrm{dequeue}(L)$}
    \STATE{$P \gets P \cup \{s\}$}
    \FOR{$(s_A \xrightarrow{a} t_A \in \delta_A) \times (s_B \xrightarrow{b} t_B \in \delta_B)$}
        \STATE{$t \gets (t_A, t_B)$}
        \IF{$t \not\in P$}
            \STATE{$L \gets L \cup \{t\}$}
            \STATE{$Q_C \gets Q_C \cup \{t\}$}
        \ENDIF
    \ENDFOR
\ENDWHILE
\end{algorithmic}
